...
UI8Kit ships four files: `base.css`, `components.css`, `shell.css`, `latty.css` (see [styles package](../packages/styles.md)). The Tailwind compiler must process them together with your app’s entry file.
...
cp "$KIT/styles/base.css" "$KIT/styles/components.css" "$KIT/styles/shell.css" "$KIT/styles/latty.css" static/css/ui8kit/
```
...
## App-level CSS sync helper

If your app includes `app/scripts/sync-ui8kit-css.sh`, run it before `npm run build:css`:
...
The sync script keeps copied files under `static/css/ui8kit/` aligned with the currently installed `github.com/fastygo/ui8kit` version.
