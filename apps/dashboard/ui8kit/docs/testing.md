...
| `styles` | `embed_test.go` | 1 | Verifies all 4 CSS files (`base.css`, `shell.css`, `components.css`, `latty.css`) are embedded and non-empty |
...
Reads each file from `embed.FS` and verifies it exists and is not empty. Catches accidental removal of CSS files or broken `//go:embed` directives (including `shell.css`).
